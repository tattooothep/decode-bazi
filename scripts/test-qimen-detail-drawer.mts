...
assertHas("หัวก้าน · 旬首", "lead stem xun-shou header label is Thai-first");
assertHas("Lead Stem · 旬首", "lead stem English label is precise");
assertHas("ปะทะ · 日時沖", "day/hour clash header label is Thai-first");
assertHas("id=\"qm-pill-clash\"", "day/hour clash header pill exists");
assertHas("ทิศ/กว้า · 方位/八卦", "palace gua row label is Thai-first");
...
assertHas("makeRow('ก้านฟ้า/ก้านดิน', '天盤干/地盤干'", "main layer includes stem layer");
...
assertBlockHas(renderHeader, "const fushi = chart.ctext_fushi || {}", "header reads ctext fushi packet");
...
assertBlockHas(renderHeader, "xunLeader ? xunLeader", "header must not append xun suffix to lead stem");
assert(/xunLeader\s*\?\s*xunLeader\s*:/.test(renderHeader), "header xun leader branch must return raw xunLeader before fallback formatting");
...
assertNotHas("六十四卦", "must not claim 64 hexagram before API sends it");
assertNotHas("Hexagram", "must not label trigram payload as hexagram");
