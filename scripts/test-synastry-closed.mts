...
ck("拱 เดี่ยวเบาสุด แต่เมื่อปี/วัยจรเติมต้องยก三合/三會ก่อน", /拱 เดี่ยวเบาสุด; ถ้าวัยจร\/ปีจรเติมกิ่งกลางของ拱 ให้ยกเป็นวงครบ三合\/三會ก่อน/.test(out), "");
...
// กลุ่ม 3 คน กิ่งไม่สัมพันธ์ (寅×寅 = ไม่ 合冲害破) + yongEls ว่าง → ไม่มี hit · กลุ่ม(≥3) ไม่บังคับโชว์ elRel → none branch
const o0 = buildSynastry([P("A", "wood", [], "寅", "寅"), P("B", "metal", [], "寅", "寅"), P("C", "earth", [], "寅", "寅")], "th");
ck("ไม่มีคู่เด่น → ยังมี header closed-list", /ลิสต์ปิด.*เทียบครบทุกคู่ 3 คู่/s.test(o0), "");
ck("ไม่มีคู่เด่น (กลุ่ม) → ข้อความ 'เช็คทุกคู่แล้ว ไม่มีเด่น'", /เช็คทุกคู่แล้ว · ไม่มีคู่ใดมีปฏิกิริยาข้ามคนเด่น/.test(o0), "");

console.log("[C · เทียบ 2 คน → โชว์ความสัมพันธ์ธาตุวันเจ้าเสมอ (比肩ไม่เงียบ · 1 คู่ ไม่ noise)]");
const cBi = buildSynastry([P("A", "wood", [], "寅", "寅"), P("B", "wood", [], "寅", "寅")], "th"); // ธาตุเดียวกัน · ไม่มี合冲
ck("C · 2 คน 比肩(ธาตุเดียวกัน) ไม่มี合冲 → ยังขึ้นคู่ + บอก 'ธาตุเดียวกัน(เพื่อน)'", /A ↔ B/.test(cBi) && /ธาตุเดียวกัน\(เพื่อน\)/.test(cBi), cBi);
ck("C · 2 คน → header มี cmpNote 'baseline...ไม่ใช่ปฏิกิริยาเด่น' (กัน AI ตีความ shown ผิด)", /baseline[\s\S]*ไม่ใช่ปฏิกิริยาเด่น/.test(cBi), "");
ck("C · กลุ่ม ≥3 คน → ไม่มี cmpNote (เฉพาะ 2 คน)", !/บรรทัดนั้นคือพื้นฐาน ไม่ใช่ปฏิกิริยาเด่น/.test(buildSynastry([P("A", "wood", [], "寅", "子"), P("ป้า", "earth", [], "午", "午"), P("ไนท์", "earth", [], "巳", "卯")], "th")), "");
ck("C · กลุ่ม 3 คน 比肩ล้วน → ไม่ flood (คู่ไม่มี合冲ไม่ขึ้น · none branch)", /ไม่มีคู่ใดมีปฏิกิริยาข้ามคนเด่น/.test(buildSynastry([P("A", "wood", [], "寅", "寅"), P("B", "wood", [], "寅", "寅"), P("C", "wood", [], "寅", "寅")], "th")), "");
...
const t3soloHits = t3solo.split("\n").filter(l=>l.startsWith("  - ")).join(" | ");
ck("申子辰 ครบในคน A คนเดียว → ไม่ขึ้น 三合 ข้ามคน", !/วงครบ/.test(t3soloHits), t3soloHits||"");
...
console.log("\n[A · เรียง hit ตามน้ำหนัก 任鐵樵: 三合 < 天干五合 < 六破]");
// A(子,申,寅) B(辰,酉,戌): 申子辰=三合水 · 壬×丁=丁壬合(五合) · 甲×己=甲己合(五合) · 子×酉=六破 → ต้องเรียง 三合→五合→破
const aOrd = buildSynastry([PM("A", "壬", "子", "庚", "申", "甲", "寅"), PM("B", "己", "辰", "丁", "酉", "甲", "戌")], "th");
const aLine = aOrd.split("\n").find((l) => l.startsWith("  - ")) || "";
const iSan = aLine.indexOf("สามฮะ"), iHe5 = aLine.indexOf("ก้านห้าฮะ"), iPo = aLine.indexOf("บั่นทอน");
ck("A · มีครบ 三合 + 天干五合 + 六破 ในคู่เดียว", iSan >= 0 && iHe5 >= 0 && iPo >= 0, aLine);
ck("A · เรียง 三合(แรงสุด) มาก่อน 天干五合", iSan >= 0 && iHe5 > iSan, aLine);
ck("A · เรียง 天干五合 มาก่อน 六破(อ่อนสุด)", iHe5 >= 0 && iPo > iHe5, aLine);

console.log("\n[เฟส 3 · 拱 resolved สำเร็จรูป + ปีจรเติมเป็น 三合]");
const tGongActivated = buildSynastry([
  { ...PM("na", "壬", "寅", "壬", "辰", "丙", "子"), annualPillar: { stem: "丙", branch: "午" } },
  PM("Mu", "戊", "戌", "丁", "亥", "甲", "卯"),
], "th");
const tGongLine = tGongActivated.split("\n").find((l) => /โอบกิ่งว่าง|HK_SYNASTRY_ACTIVATED/.test(l)) || "";
ck("寅戌拱午 → มี resolved/activated summary ให้ AI คัด ไม่ต้องตีความเอง", /HK_SYNASTRY_ACTIVATED/.test(tGongLine), tGongLine);
ck("ปี午เติม → ระบุ 寅午戌三合ไฟ", /寅午戌三合ไฟ/.test(tGongLine), tGongLine);
ck("บรรทัดเดียวกันห้ามอ่านเป็น 寅戌冲", /ห้ามอ่านเป็น 寅戌冲/.test(tGongLine), tGongLine);

console.log(`\n[synastry · import จริง · เฟส 0+1+what-if+เฟส2+AC] ${pass}/${pass + fail} passed`);
