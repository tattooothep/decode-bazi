...
const RUN_ID = `fusion5-live-partner-risk-${new Date().toISOString().replace(/[:.]/g, "-")}`;
...
const question = [
  "ปี 2028 ถ้าจะทำธุรกิจกับหุ้นส่วนคนนี้ ควรเซ็น MOU/NDA แบ่งหุ้น 40/60 หรือแบ่งรายได้ดี?",
  "เขามีประวัติส่งงานช้าและกลัว partner not delivering หรือผิดสัญญา",
  "ควร audit เงิน/เอกสารก่อน, ทำ small pilot, ใส่ buyout/exit clause หรือ terminate partnership ไหม",
  "ฟันธงให้เฉพาะดวงนี้ อย่าตอบกว้าง",
].join(" ");
...
function evaluatePanel(science, reply) {
  ...
  const common = {
    action: hasAny(text, [/small\s*pilot|pilot|นำร่อง|ทดลอง/i, /audit|ตรวจ(?:เงิน|เอกสาร|สอบ)|เอกสาร/i, /exit|buyout|terminate|ยุติ|เลิกหุ้น|ถอน|ซื้อหุ้นคืน/i]),
    partnerRisk: hasAny(text, [/หุ้นส่วน|คู่ค้า|partner|counterparty|คู่สัญญา/i]),
    contract: hasAny(text, [/MOU|NDA|สัญญา|ข้อตกลง|contract|clause|เงื่อนไข/i]),
    moneySplit: hasAny(text, [/แบ่งหุ้น|equity|แบ่งรายได้|revenue|profit|40\/60|เงิน|รายได้|ทรัพย์|財|2nd|11th/i]),
    breachExit: hasAny(text, [/ผิดสัญญา|breach|not delivering|ส่งงานช้า|fraud|โกง|liability|exit|terminate|ยุติ|風險|风险/i]),
  };
  const scienceChecks = {
    qizheng: {
      owner: hasAny(text, [/三主|命度|度主|身主|เจ้าชะตา|ตัวเจ้าของ/i]),
      partner: hasAny(text, [/妻妾|คู่สัญญา|หุ้นส่วน|partner/i]),
      authority: hasAny(text, [/官祿|อำนาจตัดสินใจ|สิทธิ์ตัดสินใจ|บทบาท/i]),
      asset: hasAny(text, [/財帛|田宅|แบ่งหุ้น|แบ่งรายได้|ทรัพย์|asset/i]),
      docs: hasAny(text, [/兄弟|เอกสาร|ข้อความ|หลักฐาน|MOU|NDA/i]),
      risk: hasAny(text, [/疾厄|仇|難|ความเสี่ยง|โกง|ผิดสัญญา|liability/i]),
      timing: hasAny(text, [/行限|流年|ปี\s*2028|จังหวะ|timing/i]),
    },
    ziwei: {
      owner: hasAny(text, [/命宮|身宮|เจ้าชะตา|ตัวเจ้าของ/i]),
      partner: hasAny(text, [/夫妻|คู่สัญญา|หุ้นส่วน|partner/i]),
      authority: hasAny(text, [/官祿|อำนาจตัดสินใจ|สิทธิ์ตัดสินใจ|บทบาท/i]),
      asset: hasAny(text, [/財帛|田宅|แบ่งหุ้น|แบ่งรายได้|ทรัพย์|asset/i]),
      docs: hasAny(text, [/父母|兄弟|เอกสาร|หลักฐาน|MOU|NDA/i]),
      risk: hasAny(text, [/疾厄|化忌|煞曜|ความเสี่ยง|โกง|ผิดสัญญา|liability/i]),
    ...
    western: {
      ...
      partner: hasAny(text, [/7th|เรือน\s*7|Venus|ศุกร์|partner|คู่สัญญา|หุ้นส่วน/i]),
      authority: hasAny(text, [/MC|10th|เรือน\s*10|Saturn|อำนาจตัดสินใจ|บทบาท/i]),
      asset: hasAny(text, [/2nd|8th|Fortune|เรือน\s*2|เรือน\s*8|แบ่งหุ้น|แบ่งรายได้|ทรัพย์/i]),
      docs: hasAny(text, [/3rd|9th|Mercury|พุธ|เอกสาร|หลักฐาน|MOU|NDA/i]),
      risk: hasAny(text, [/6th|8th|12th|Mars|Saturn|ความเสี่ยง|ผิดสัญญา|liability/i]),
      timing: hasAny(text, [/transit|return|ปี\s*2028|จังหวะ|timing/i]),
    },
    vedic: {
      owner: hasAny(text, [/Lagna|Moon|ลัคนา|จันทร์|ตัวเจ้าของ/i]),
      partner: hasAny(text, [/7th|Venus|Jupiter|ศุกร์|พฤหัส|คู่สัญญา|หุ้นส่วน/i]),
      authority: hasAny(text, [/10th|D10|เรือน\s*10|อำนาจตัดสินใจ|บทบาท/i]),
      asset: hasAny(text, [/2nd|11th|8th|เรือน\s*2|เรือน\s*11|เรือน\s*8|แบ่งหุ้น|แบ่งรายได้|ทรัพย์/i]),
      docs: hasAny(text, [/3rd|9th|Mercury|พุธ|เอกสาร|หลักฐาน|MOU|NDA/i]),
      risk: hasAny(text, [/6th|8th|12th|Rahu|Ketu|Mars|Saturn|ความเสี่ยง|ผิดสัญญา|liability/i]),
      timing: hasAny(text, [/dasha|gochara|ทศา|โคจร|ปี\s*2028|จังหวะ|timing/i]),
    },
  }[science] || {};
  const canonAnchorCount = Object.values(scienceChecks).filter(Boolean).length;
  const checks = { ...common, ...scienceChecks };
  const missing = Object.entries(checks).filter(([, ok]) => !ok).map(([name]) => name);
  if (canonAnchorCount < 6) missing.push("canonPacketAnchors");
  return {
