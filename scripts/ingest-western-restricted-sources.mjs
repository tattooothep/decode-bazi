...
const base = join(root, "private/restricted-sources/western");
const inputDirs = [
...
const manifest = {
  ...
  policy: {
    rawFilesStayPrivate: "private/restricted-sources/western",
...
writeFileSync(
  ...
  [
    "status\tbucket\trawBytes\textractedBytes\tmethod\trawPath\textractedPath\terror",
    ...rows.map((r) => [r.status, r.bucket, r.rawBytes, r.extractedBytes, r.method || "", r.rawPath, r.extractedPath, r.error || ""].join("\t")),
  ].join("\n") + "\n",
);
console.log(`Ingested ${manifest.counts.ok}/${manifest.counts.total} western source files`);
