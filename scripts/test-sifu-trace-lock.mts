...
import { extractTraceFacts, parseTraceLine, stripTraceLine, validateTrace, parseClaimedSources } from "../src/lib/sifu-trace-lock";
...
/* ⟦SRC⟧ self-report (13 มิ.ย.) */
const FULL = "⟦ID⟧日干=壬⟧\n⟦TRACE⟧從=ไม่มี·格局=雜氣劫財格·用神=ไฟ·ตำรา=子平真詮|窮通寶鑑·เทรน=คำแนะนำอาชีพ⟧\nเนื้อ...";
const cs = parseClaimedSources(FULL);
ok("SRC: แกะเล่มได้ 2 เล่ม", cs?.books.length === 2 && cs.books[0] === "子平真詮", JSON.stringify(cs));
ok("SRC: แกะส่วนเทรนได้", cs?.trained === "คำแนะนำอาชีพ");
const cs2 = parseClaimedSources("⟦TRACE⟧從=ไม่มี·格局=X格·用神=ไฟ·ตำรา=A·เทรน=ไม่มี⟧");
ok("SRC: เทรน=ไม่มี → null", cs2?.trained === null && cs2?.books.length === 1);
ok("SRC: รูปเก่าไม่มีช่องตำรา → null (ไม่พัง)", parseClaimedSources(GOOD) === null);
ok("TRACE รูปขยายยัง validate ผ่าน (用神 ไม่ดูดช่องตำรา)", validateTrace(FULL, facts).ok);
ok("strip รูปขยายเกลี้ยง", !stripTraceLine(FULL).includes("⟦TRACE⟧"));
