...
try {
  ...
  for (const fixture of fixtures) {
    ...
    check((r.data.entitlement?.allowed_intents || []).length === (caps.calendar.intents === "one" ? 1 : 16), `${fixture.plan} calendar intents`);
    ...
    r = await request(fixture, `/api/mobile/v1/today?date=${date}&profileId=${fixture.profileId}`, {}, true);
    check(r.response.status === 200 && r.data.entitlement?.day_window === caps.today.day_window, `${fixture.plan} mobile Today returns authoritative day window`);
    ...
    r = await request(fixture, `/api/mobile/v1/chart/${fixture.profileId}`, {}, true);
