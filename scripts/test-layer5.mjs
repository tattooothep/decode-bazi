...
// ชั้น D: ZPZQ 5.2 resolver — ปิด 3 fail จาก audit 81 命例 (合煞留官 / 合煞存財 / 印護)
console.log('\n=== ชั้น D: ZPZQ 5.2 resolver regression (3 fail closures) ===');
{
  const cases = [
    // [name, pillars, dm, ge, expectVerdict, expectSubLabel, reasonHas, timingLuck, expectTiming]
  ...
  for (const [name, pil, dm, ge, exp, subExp, reasonHas, luck, timingExp] of cases) {
    ...
    const gotSub = r?.subLabel ?? null;
    const cb = buildChengBaiNow(r, dm, luck, null, 'rooted', false);
    const gotTiming = cb === null ? 'NULL' : cb.verdict;
    const ok = r?.verdict === exp && gotSub === subExp && (r.reason || '').includes(reasonHas) && gotTiming === timingExp;
    console.log(`${ok?'✓':'✗'} ${name} → ${r?.verdict}/${gotSub ?? 'null'} · timing=${gotTiming} · ${r?.reason} (expect ${exp}/${subExp ?? 'null'}+${reasonHas}+${timingExp})`);
    ok ? pass++ : fail++;
  }
  // guard: อย่าให้ resolver 財格 กว้างเกินจนเปลี่ยน 財格+食生 เป็น 帶傷 timing key
  const fg = buildXiangShen(P('庚午','己午','甲子','丙午'), '甲', '財格');
