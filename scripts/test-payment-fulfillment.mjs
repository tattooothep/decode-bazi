...
try {
  ...
  const refund = await refundPaidOrder(orderId, "automated refund regression");
  check(refund.ok && refund.clawback.ok, "refund claws back the paid order");
  const refunded = await client.query(
    `SELECT u.tier,u.hour_balance,s.status AS source_status
       FROM users u
       LEFT JOIN product_entitlement_sources s
         ON s.user_id=u.id AND s.source_kind='web' AND s.source_ref=$2
      WHERE u.id=$1`,
    [userId, `order:${orderId}`]
  );
  check(refunded.rows[0].source_status === "revoked", "refund revokes only the order entitlement source");
  check(refunded.rows[0].tier === "free", "refund recomputes the user tier after source revocation");
  check(Number(refunded.rows[0].hour_balance) === 1000, "refund restores the pre-order balance");

  const repeatRefund = await refundPaidOrder(orderId, "automated refund replay");
  check(repeatRefund.ok && repeatRefund.clawback.status === "already", "repeat refund is idempotent");
} finally {
  if (orderId) {
