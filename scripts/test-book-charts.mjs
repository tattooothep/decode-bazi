 ...
 *   [3] pricing computeBookYam — 50×n + (หลอมรวม && n≥2 ? 50 : 0)
 */
...
const { computeBookYam } = await import("../src/app/api/book/route.ts");
...
console.log("[3] pricing 50×n + (หลอมรวม && n≥2 ? 50 : 0)");
const ALL6 = ["bazi", "ziwei", "qizheng", "western", "vedic", "uranian"];
ok(computeBookYam(ALL6, true) === 350, "6 ศาสตร์ + หลอมรวม = 350", "got " + computeBookYam(ALL6, true));
ok(computeBookYam(ALL6, false) === 300, "6 ศาสตร์ ไม่หลอมรวม = 300", "got " + computeBookYam(ALL6, false));
ok(computeBookYam(["bazi", "western"], true) === 150, "2 ศาสตร์ + หลอมรวม = 150", "got " + computeBookYam(["bazi", "western"], true));
ok(computeBookYam(["bazi", "western"], false) === 100, "2 ศาสตร์ ไม่หลอมรวม = 100", "got " + computeBookYam(["bazi", "western"], false));
ok(computeBookYam(["bazi"], true) === 50, "1 ศาสตร์ + ติ๊กหลอมรวม = 50 (≥2 guard)", "got " + computeBookYam(["bazi"], true));
ok(computeBookYam([], true) === 0, "ว่าง = 0", "got " + computeBookYam([], true));
ok(computeBookYam(["bazi", "notreal"], true) === 50, "กรองศาสตร์ที่ไม่ available (1 ตัวจริง = 50)", "got " + computeBookYam(["bazi", "notreal"], true));
