...
exports.buildChartExtensions = buildChartExtensions;
const tyme = __importStar(require("tyme4ts"));
/* 19 พ.ค. Option α · accept hour=null · 4p path byte-equal · 3p path: skip hour-only events */
const ACTIVE_KEYS_4P = ["year", "month", "day", "hour"];
function activeKeys(p) {
    return ACTIVE_KEYS_4P.filter(k => p[k] != null);
}
const STEM_ELEMENT = {
...
function tenGodOf(dayMaster, targetStem) {
    ...
    if (ELEMENT_PRODUCES[dmEl] === tEl)
        return samePol ? "食神" : "傷官";
    if (ELEMENT_CONTROLS[dmEl] === tEl)
        return samePol ? "偏財" : "正財";
    if (ELEMENT_CONTROLS[tEl] === dmEl)
        return samePol ? "七殺" : "正官";
    if (ELEMENT_PRODUCES[tEl] === dmEl)
...
function buildTenGodsMap(pillars) {
...
function buildQiPhases(pillars) {
    const dm = pillars.day.stem;
    return {
...
function buildStemInteractions(pillars) {
    const pkeys = activeKeys(pillars);
    ...
    for (let i = 0; i < pkeys.length; i++) {
        for (let j = i + 1; j < pkeys.length; j++) {
            ...
            if (STEM_CLASH[a] === b) {
                out.push({ type: "天克", pair: [a, b], pillars_pair: [pkeys[i], pkeys[j]] });
            }
            ...
            if (info && info.partner === b) {
                ...
                if (!seenCombo.has(key)) {
                    ...
                    out.push({ type: "五合", pair: [a, b], pillars_pair: [pkeys[i], pkeys[j]], element: info.element, transformed });
                }
            }
        }
    }
...
function classifyFanFu(a, b) {
    ...
    if (sameStem && sameBranch)
        return "伏吟"; /* ซ้ำเต็ม */
    if (clashStem && clashBranch)
        return "反吟"; /* พลิกเต็ม */
    if (sameStem && clashBranch)
        return "反吟·ก้าน"; /* ก้านซ้ำ + กิ่งชน */
    if (clashStem && sameBranch)
        return "反吟·กิ่ง"; /* ก้านชน + กิ่งซ้ำ */
    if (sameStem)
        return "伏吟·ก้าน"; /* ก้านซ้ำเดี่ยว */
    if (sameBranch)
        return "伏吟·กิ่ง"; /* กิ่งซ้ำเดี่ยว */
    return null;
}
function buildFanYinFuYin(pillars, luckPillars, currentLuckIdx, currentYear) {
    const out = [];
    const pkeys = activeKeys(pillars);
    ...
    for (let i = 0; i < pkeys.length; i++) {
        for (let j = i + 1; j < pkeys.length; j++) {
            ...
            if (t)
                out.push({ type: t, natal_pillar: pkeys[i], other_pillar: pkeys[j], natal: a, other: b });
        }
    }
    ...
    if (currentLuckIdx >= 0 && luckPillars[currentLuckIdx]) {
        ...
        for (const pk of pkeys) {
            ...
            if (t)
                out.push({ type: t, natal_pillar: pk, other_pillar: lpKey, natal: n, other: lpStem });
        }
    }
    ...
    if (currentYear) {
        for (const pk of pkeys) {
            ...
            if (t)
                out.push({ type: t, natal_pillar: pk, other_pillar: "current_year", natal: n, other: currentYear });
        }
    }
    return out;
}
function buildVoytekStrength(pillars, _elementCounts) {
    const dm = pillars.day.stem;
    const dmEl = STEM_ELEMENT[dm];
    const producesDm = Object.keys(ELEMENT_PRODUCES).find(k => ELEMENT_PRODUCES[k] === dmEl);
    const wealth = ELEMENT_CONTROLS[dmEl];
    const power = Object.keys(ELEMENT_CONTROLS).find(k => ELEMENT_CONTROLS[k] === dmEl);
    const output = ELEMENT_PRODUCES[dmEl];
    ...
    for (const k of pkeys) {
        ...
        for (let i = 0; i < hs.length; i++) {
            const he = STEM_ELEMENT[hs[i]];
            if (he)
                counts[he] += weights[i] || 0.1;
        }
    }
    const total = counts.wood + counts.fire + counts.earth + counts.metal + counts.water || 1;
    const pct = (el) => Math.round((counts[el] / total) * 100);
    const supporting = pct(dmEl) + pct(producesDm);
    const opposing = 100 - supporting;
    let level;
    let level_th, level_en, level_zh;
    if (supporting <= 15) {
        level = "extreme_weak";
        level_th = "อ่อนสุดขีด";
        level_en = "Extreme Weak";
        level_zh = "極弱";
    }
    else if (supporting <= 35) {
        level = "weak";
        level_th = "อ่อน";
        level_en = "Weak";
        level_zh = "弱";
    }
    else if (supporting <= 45) {
        level = "balanced_weak";
        level_th = "สมดุล-อ่อน";
        level_en = "Balanced-Weak";
        level_zh = "中和偏弱";
    }
    else if (supporting <= 55) {
        level = "balanced";
        level_th = "สมดุล";
        level_en = "Balanced";
        level_zh = "中和";
    }
    else if (supporting <= 65) {
        level = "balanced_strong";
        level_th = "สมดุล-แกร่ง";
        level_en = "Balanced-Strong";
        level_zh = "中和偏強";
    }
    else if (supporting <= 85) {
        level = "strong";
        level_th = "แกร่ง";
        level_en = "Strong";
        level_zh = "強";
        ...
        level_th = "แกร่งสุดขีด";
        level_en = "Extreme Strong";
        level_zh = "極強";
    }
    const ROLE_META = {
...
function currentYearPillar(date) {
...
function buildLpNatalInteractions(pillars, lp) {
    const pkeys = activeKeys(pillars);
    const out = [];
    for (const pk of pkeys) {
        const n = pillars[pk];
        const types = [];
        /* Stem */
        if (STEM_CLASH[n.stem] === lp.stem)
            types.push("天克");
        const sc = STEM_COMBOS[n.stem];
        if (sc && sc.partner === lp.stem)
            types.push("五合");
        /* Branch */
        if (SIX_CLASH[n.branch] === lp.branch)
            types.push("六沖");
        if (SIX_HE[n.branch] === lp.branch)
            types.push("六合");
        if (SIX_HARM[n.branch] === lp.branch)
            types.push("六害");
        if (SIX_DESTROY[n.branch] === lp.branch)
            types.push("六破");
        ...
        if (types.length)
            out.push({ natal_pillar: pk, types, natal: n, lp });
    }
...
function buildTianDiHe(pillars) {
    const pkeys = activeKeys(pillars);
    const out = [];
    for (let i = 0; i < pkeys.length; i++) {
        for (let j = i + 1; j < pkeys.length; j++) {
...
function buildSpecialChartRules(pillars, geJuStructure) {
    const dm = pillars.day.stem;
    const dmEl = STEM_ELEMENT[dm];
    ...
    if (/^化[木火土金水]格$/.test(structure)) {
        ...
        const el = { 木: "wood", 火: "fire", 土: "earth", 金: "metal", 水: "water" }[zhEl];
        const child = ELEMENT_PRODUCES[el];
        const parent = Object.keys(ELEMENT_PRODUCES).find(k => ELEMENT_PRODUCES[k] === el) || "";
        return {
            applicable: true,
...
const BRANCH_TH = { 子: "ชวด", 丑: "ฉลู", 寅: "ขาล", 卯: "เถาะ", 辰: "มะโรง", 巳: "มะเส็ง", 午: "มะเมีย", 未: "มะแม", 申: "วอก", 酉: "ระกา", 戌: "จอ", 亥: "กุน" };
function buildSpousePalace(pillars) {
    ...
    for (const o of otherBranches) {
        if (SIX_CLASH[dayBr] === o.branch)
            flags.push(`六沖·${o.pillar}`);
        if (SIX_HE[dayBr] === o.branch)
            flags.push(`六合·${o.pillar}`);
        if (SIX_HARM[dayBr] === o.branch)
            flags.push(`六害·${o.pillar}`);
    }
    const TRAITS_TH = {
    ...
    const BRANCH_EN = { 子: "Rat", 丑: "Ox", 寅: "Tiger", 卯: "Rabbit", 辰: "Dragon", 巳: "Snake", 午: "Horse", 未: "Goat", 申: "Monkey", 酉: "Rooster", 戌: "Dog", 亥: "Pig" };
    const ELEMENT_EN_LOCAL = { wood: "Wood", fire: "Fire", earth: "Earth", metal: "Metal", water: "Water" };
    const ELEMENT_ZH_LOCAL = { wood: "木", fire: "火", earth: "土", metal: "金", water: "水" };
    return {
        day_branch: dayBr,
...
function buildCareerIndustry(pillars, yongshenElement) {
    ...
    return {
        ...
        advice_zh: `${{ wood: "木", fire: "火", earth: "土", metal: "金", water: "水" }[pickEl] || pickEl}行事業與命局相合 · 生扶用神`,
    };
}
const ELEMENT_TH_MAP_LOCAL = { wood: "ไม้", fire: "ไฟ", earth: "ดิน", metal: "ทอง", water: "น้ำ" };
...
const ELEMENT_EN_MAP_LOCAL = { wood: "Wood", fire: "Fire", earth: "Earth", metal: "Metal", water: "Water" };
const ELEMENT_ZH_MAP_LOCAL = { wood: "木", fire: "火", earth: "土", metal: "金", water: "水" };
function buildHealthMapping(pillars, elementCounts, strengthPct) {
    const dmEl = STEM_ELEMENT[pillars.day.stem];
...
function hiddenStemDetails(branch, dm, usefulRole) {
...
function baziYearBoundary(year) {
    try {
...
function buildLiuNianTimeline(pillars, baseYear, span = 10, birthYear) {
    const dm = pillars.day.stem;
    ...
    for (let i = 0; i < span; i++) {
        ...
        const vs = [];
        if (SIX_CLASH[dayBranch] === branch)
            vs.push("六沖");
        if (SIX_HE[dayBranch] === branch)
            vs.push("六合");
        if (SIX_HARM[dayBranch] === branch)
            vs.push("六害");
        if (SIX_DESTROY[dayBranch] === branch)
            vs.push("六破");
        for (const r of BAN_HE_PAIRS) {
        ...
        /* Simple flag */
        let flag = "neutral";
        if (vs.some(t => t === "六合" || t.startsWith("半合")))
            flag = "auspicious";
        else if (vs.some(t => t === "六沖" || t === "六害" || t === "六破"))
            flag = "cautious";
        /* hex per year */
        let hex = null;
...
function branchTransitTags(dayBranch, branch) {
    const vs = [];
    if (SIX_CLASH[dayBranch] === branch)
        vs.push("六沖");
    if (SIX_HE[dayBranch] === branch)
...
function transitFlag(vs) {
    if (vs.some(t => t === "六沖" || t === "六害" || t === "六破"))
        return "cautious";
    if (vs.some(t => t === "六合" || t.startsWith("半合")))
        return "auspicious";
    return "neutral";
}
const TRANSIT_TYPE_TH = {
    伏吟: "เสาซ้ำเรื่องเดิม",
    反吟: "เสาพลิกแรง",
    六沖: "ปะทะ",
    六合: "จับคู่",
...
function transitStrength(type, target) {
    if ((type === "伏吟" || type === "反吟") && (target === "day" || target === "luck"))
        return "critical";
    if (type === "伏吟" || type === "反吟")
        return "high";
    if (type === "六沖" && (target === "day" || target === "month"))
        return "high";
    if (type === "六沖")
        return "medium";
    if (type === "六害" || type === "六破" || type === "天克")
        return "medium";
    return "low";
}
function buildTransitImpacts(pillars, transit, luck) {
    const out = [];
    const pushImpact = (typeRaw, target, pair) => {
    ...
    const targets = [
        ...activeKeys(pillars).map((key) => ({ key: key, stem: pillars[key].stem, branch: pillars[key].branch })),
        ...(luck ? [{ key: "luck", stem: luck.stem, branch: luck.branch }] : []),
    ];
    for (const t of targets) {
        const full = classifyFanFu({ stem: t.stem, branch: t.branch }, transit);
        if (full === "伏吟" || full === "反吟")
            pushImpact(full, t.key, `${t.stem}${t.branch}↔${transit.stem}${transit.branch}`);
        for (const tag of branchTransitTags(t.branch, transit.branch)) {
            const base = tag.split("·")[0];
            pushImpact(base, t.key, `${t.branch}${transit.branch}`);
        }
        if (STEM_CLASH[t.stem] === transit.stem)
            pushImpact("天克", t.key, `${t.stem}${transit.stem}`);
        const combo = STEM_COMBOS[t.stem];
        if (combo?.partner === transit.stem)
            pushImpact("五合", t.key, `${t.stem}${transit.stem}`);
    }
    return out.sort((a, b) => {
...
function addAgeYears(birthDate, age) {
...
function monthPillarMidMonth(year, month) {
...
function liuYuePillarByJieqi(year, month) {
    try {
...
function buildLiuYueForYear(pillars, year, luck, usefulRole) {
    const dm = pillars.day.stem;
    const dayBranch = pillars.day.branch;
    const out = [];
...
function annotateLuckPillarTiming(lp, birthDate) {
    if (!birthDate)
        return lp;
...
function buildLuckDecadeDrilldown(pillars, lp, birthYear, usefulRole) {
    if (!birthYear)
        return [];
    return lp.map((p, idx) => {
        ...
        for (let y = candidateStart - 1; y <= candidateEnd + 1; y++) {
            ...
            if (luckStart && luckEnd && boundary.startDate && boundary.endDate) {
                if (boundary.startDate < luckEnd && boundary.endDate > luckStart)
                    yearNums.push(y);
            }
...
function buildInteractions(pillars) {
    const pkeys = activeKeys(pillars);
    const out = [];
    for (let i = 0; i < pkeys.length; i++) {
        for (let j = i + 1; j < pkeys.length; j++) {
            ...
            if (SIX_CLASH[a] === b)
                out.push({ type: "六沖", pair: [a, b], pillars_pair: [pkeys[i], pkeys[j]] });
            if (SIX_HE[a] === b)
                out.push({ type: "六合", pair: [a, b], pillars_pair: [pkeys[i], pkeys[j]] });
            if (SIX_HARM[a] === b)
                out.push({ type: "六害", pair: [a, b], pillars_pair: [pkeys[i], pkeys[j]] });
            if (SIX_DESTROY[a] === b)
                out.push({ type: "六破", pair: [a, b], pillars_pair: [pkeys[i], pkeys[j]] });
        }
    }
    return out;
}
function buildPunishments(pillars) {
    ...
    if (ungrateful.length >= 2)
        flags.push({ type: "三刑", branches: ungrateful });
    const earth = ["丑", "戌", "未"].filter((b) => branches.includes(b));
    if (earth.length >= 2)
        flags.push({ type: "三刑", branches: earth });
    const selfSet = ["辰", "午", "酉", "亥"];
    for (const s of selfSet) {
        if (branches.filter((b) => b === s).length >= 2)
            flags.push({ type: "自刑", branches: [s, s] });
    }
    if (branches.includes("子") && branches.includes("卯"))
        flags.push({ type: "子卯刑", branches: ["子", "卯"] });
    return flags;
}
function buildCombinations(pillars) {
    const pkeys = activeKeys(pillars);
    const branches = pkeys.map(k => pillars[k].branch);
    const san_he = [];
    for (const set of SAN_HE_SETS) {
        ...
        for (let j = i + 1; j < pkeys.length; j++) {
            ...
            for (const rule of BAN_HE_PAIRS) {
                if ((rule.pair[0] === a && rule.pair[1] === b) || (rule.pair[0] === b && rule.pair[1] === a)) {
                    ban_he.push({ pair: [a, b], element: rule.element, pillars_pair: [pkeys[i], pkeys[j]] });
                }
            }
        }
    }
...
function buildJishen(dayMaster, adjustedYongshenElements) {
    ...
    let wealth_role = "neutral";
    if (usefulElements.includes(wealthElement))
        wealth_role = "yongshen";
    else if (jishen.includes(wealthElement))
        wealth_role = "jishen";
    return { elements: jishen, wealth_role };
}
function buildTodayOverlay(pillars, todayDate) {
    try {
        ...
        const types = [];
        if (SIX_CLASH[dayBranch] === tb)
            types.push("六沖");
        if (SIX_HE[dayBranch] === tb)
            types.push("六合");
        if (SIX_HARM[dayBranch] === tb)
            types.push("六害");
        if (SIX_DESTROY[dayBranch] === tb)
            types.push("六破");
        return {
            today_pillar,
...
function buildLuckPillars(pillars, gender = "M", startAge = 10) {
    ...
    if (monthStemIdx < 0 || monthBranchIdx < 0)
        return [];
    const dm = pillars.day.stem;
    const out = [];
    for (let i = 1; i <= 8; i++) {
        ...
        const element = STEM_ELEMENT[stem] || "earth";
...
const chart_personal_stars_1 = require("./chart-personal-stars");
function buildChartExtensions(pillars, todayDate = new Date(), gender = "M", birthDate, startAge = 10, geJuStructure = null, strengthPct = 50, yongshenElement = null, adjustedYongshenElements = null) {
    const pAny = pillars;
    const birthYear = birthDate?.getUTCFullYear();
    ...
    const transitUsefulRole = (element) => {
        const el = String(element || "").toLowerCase();
        if (yongEl && el === yongEl)
            return "yong";
        if (usefulList.includes(el))
            return "xi";
        if (jishenInfo.elements.includes(el))
            return "ji";
        return "neutral";
    };
    return {
        ...
        /* G1 · table data */
        nayin: (0, chart_table_1.buildNayin)(pAny),
        kong_wang: (0, chart_table_1.buildKongWang)(pAny),
        three_phases: (0, chart_table_1.buildThreePhases)(pAny),
        special_stars: (0, chart_table_1.buildSpecialStars)(pAny),
        /* G4 · 4 BaZi natal sections */
        life_palace: (0, chart_table_1.buildLifePalace)(pAny),
        palace_readings: (0, chart_table_1.buildPalaceReadings)(pAny),
        five_structure: (0, chart_table_1.buildFiveStructure)(pAny),
        personal_stars: (0, chart_personal_stars_1.buildPersonalStars)(pAny, (0, chart_table_1.buildKongWang)(pAny).per_pillar),
