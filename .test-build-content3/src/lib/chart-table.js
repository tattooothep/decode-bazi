...
exports.naYinOf = naYinOf;
exports.twelvePhaseOf = twelvePhaseOf;
exports.kongWangOf = kongWangOf;
exports.detectShenShaStars = detectShenShaStars;
exports.detectShenSha62 = detectShenSha62;
exports.buildNayin = buildNayin;
exports.buildKongWang = buildKongWang;
exports.buildThreePhases = buildThreePhases;
exports.buildSpecialStars = buildSpecialStars;
exports.buildLifePalace = buildLifePalace;
exports.buildBodyPalace = buildBodyPalace;
exports.computeSiLingDays = computeSiLingDays;
exports.buildSiLing = buildSiLing;
exports.buildMinorLuck = buildMinorLuck;
exports.buildConceptionPalace = buildConceptionPalace;
exports.buildPalaceReadings = buildPalaceReadings;
exports.buildFiveStructure = buildFiveStructure;
const hourkey_na_yin_60_json_1 = __importDefault(require("../../data/hourkey-v3/hourkey-na-yin-60.json"));
/* 19 พ.ค. Option α · active position helper (4p path byte-equal · 3p filters hour) */
const _ACTIVE_4P = ["year", "month", "day", "hour"];
function _activeKeys(p) {
    return _ACTIVE_4P.filter(k => p[k] != null);
}
const STEMS_ALL = ["甲", "乙", "丙", "丁", "戊", "己", "庚", "辛", "壬", "癸"];
const BRANCHES_ALL = ["子", "丑", "寅", "卯", "辰", "巳", "午", "未", "申", "酉", "戌", "亥"];
const STEM_ANCHOR_MAP = {
...
function naYinOf(pillar) {
    const d = hourkey_na_yin_60_json_1.default[pillar];
    if (!d)
        return null;
    return { zh: d.na_yin, en: d.en, element: d.element, symbol: d.symbol };
}
/* ── 12 phases (DM phase · pillar phase · hidden phase) ───── */
function twelvePhaseOf(stem, branch) {
    const a = STEM_ANCHOR_MAP[stem];
    if (!a)
        return null;
...
function kongWangOf(stem, branch) {
...
function detectShenShaStars(pillars) {
    ...
    const T_DB = {}; /* moved to T_TRIO · checked from both YP+DP */
    const T_YB = {
    ...
    const pp = (p) => pillars[p];
    const found = [];
    const checkBranch = (code, targets) => {
        const hits = positions.filter(p => targets.includes(pp(p).branch));
        if (hits.length)
            found.push({ code, ...META[code], pillars: [...hits] });
    };
    /* Codex fix: 天德/月德 target ตามตำราอาจเป็น stem หรือ branch · ต้อง check ทั้งคู่ */
    const checkStemOrBranch = (code, targets) => {
        const hits = positions.filter(p => targets.some(t => {
            if (STEMS_ALL.includes(t))
                return pp(p).stem === t;
            if (BRANCHES_ALL.includes(t))
                return pp(p).branch === t;
            return false;
        }));
        if (hits.length)
            found.push({ code, ...META[code], pillars: [...hits] });
    };
    for (const [code, t] of Object.entries(T_DS))
        checkBranch(code, t[ds] || []);
    for (const [code, t] of Object.entries(T_DB))
        checkBranch(code, t[db] || []);
    for (const [code, t] of Object.entries(T_YB))
        checkBranch(code, t[yb] || []);
    for (const [code, t] of Object.entries(T_MB_STEM))
        checkStemOrBranch(code, t[mb] || []);
    ...
    for (const [code, tbl] of Object.entries(T_TRIO)) {
        ...
        if (ypHits.length)
            found.push({ code, ...META[code], pillars: [...ypHits], anchor: "YP" });
    ...
    if (tgHits.length)
        found.push({ code: "tianGuan", ...META.tianGuan, pillars: [...tgHits], anchor: "DS" });
    /* +10 ดาวใหม่ · year-branch driven */
    for (const [code, tbl] of Object.entries(T_NEW_YB)) {
        const targets = tbl[yb] || [];
        const hits = positions.filter(p => targets.includes(pp(p).branch));
        if (hits.length)
            found.push({ code, ...META[code], pillars: [...hits], anchor: "YP" });
    }
    ...
    for (const [code, tbl] of Object.entries(T_NEW_DS)) {
        ...
        const hits = positions.filter(p => targets.includes(pp(p).branch));
        if (hits.length)
            found.push({ code, ...META[code], pillars: [...hits], anchor: "DS" });
    }
    ...
    if (stHits.length)
        found.push({ code: "sanTai", ...META.sanTai, pillars: [...stHits], anchor: "YP" });
    /* 天赦 · day pillar match */
...
function detectShenSha62(pillars) {
    ...
    const branchOffset = (start, n) => BRANCHES_ALL[(BRANCHES_ALL.indexOf(start) + n + 12) % 12];
    const META_EXT = {
    ...
    const push = (code, hits, anchor) => {
        if (hits.length === 0)
            return;
        const m = META_EXT[code];
        if (!m)
            return;
    ...
    if (YANG_CHA_DAYS.includes(dayPillar))
        push("yangCha", ["day"], "DP");
    if (YIN_CUO_DAYS.includes(dayPillar))
        push("yinCuo", ["day"], "DP");
    const GU_LUAN_DAYS = ["乙巳", "丁巳", "辛亥", "戊申", "甲寅", "丙午", "戊午", "壬子"];
    if (GU_LUAN_DAYS.includes(dayPillar))
        push("guLuan", ["day"], "DP");
    /* 祿庫 · static wealth-vault marker only; mukuStates decides open/closed/useful/harmful. */
    const LU_KU_BY_DAY_STEM = {
    ...
    if (yangRenBr) {
        const sixClash = { "子": "午", "午": "子", "丑": "未", "未": "丑", "寅": "申", "申": "寅", "卯": "酉", "酉": "卯", "辰": "戌", "戌": "辰", "巳": "亥", "亥": "巳" };
    ...
    if (heaven3.every(s => stemsSet.has(s)))
        push("sanQi", ["year", "month", "day"]);
    else if (earth3.every(s => stemsSet.has(s)))
        push("sanQi", ["year", "month", "day"]);
    else if (people3.every(s => stemsSet.has(s)))
        push("sanQi", ["year", "month", "day"]);
    /* G. 桃花殺 */
    const taoShaTbl = { "寅": ["卯"], "午": ["卯"], "戌": ["卯"], "巳": ["午"], "酉": ["午"], "丑": ["午"], "申": ["酉"], "子": ["酉"], "辰": ["酉"], "亥": ["子"], "卯": ["子"], "未": ["子"] };
    const tsTargets = taoShaTbl[yb] || [];
    const tsHits = positions.filter(p => tsTargets.includes(pillars[p].branch));
    if (tsHits.length > 0)
        push("taoHuaSha", tsHits);
    /* H. 元辰 */
    const yuanChenTbl = { "子": "未", "丑": "午", "寅": "酉", "卯": "申", "辰": "亥", "巳": "戌", "午": "丑", "未": "子", "申": "卯", "酉": "寅", "戌": "巳", "亥": "辰" };
...
function buildNayin(pillars) {
    const ks = _activeKeys(pillars);
    const out = { year: null, month: null, day: null, hour: null };
    for (const k of ks) {
        const pp = pillars[k];
...
function buildKongWang(pillars) {
    ...
    const inDay = (b) => b === dv1 || b === dv2;
    const inYear = (b) => b === yv1 || b === yv2;
    const inAny = (b) => inDay(b) || inYear(b);
    return {
...
function buildThreePhases(pillars) {
    const ks = _activeKeys(pillars);
    const dmStem = pillars.day.stem;
    const out = { year: null, month: null, day: null, hour: null };
    for (const k of ks) {
        const pp = pillars[k];
        const mainHidden = (exports.HIDDEN_STEMS[pp.branch] || [])[0] || null;
        out[k] = {
...
function buildSpecialStars(pillars) {
...
const ELEMENT_TH_MAP = { wood: "ไม้", fire: "ไฟ", earth: "ดิน", metal: "ทอง", water: "น้ำ" };
const BRANCH_TH_MAP = { 子: "ชวด", 丑: "ฉลู", 寅: "ขาล", 卯: "เถาะ", 辰: "มะโรง", 巳: "มะเส็ง", 午: "มะเมีย", 未: "มะแม", 申: "วอก", 酉: "ระกา", 戌: "จอ", 亥: "กุน" };
/* 五虎遁年起月 · ก้านของกิ่งใดๆ จากก้านปี (寅เป็นจุดเริ่ม) · ใช้กับ 命宮/身宮
 * 甲己→丙寅起 · 乙庚→戊寅 · 丙辛→庚寅 · 丁壬→壬寅 · 戊癸→甲寅 */
function fiveTigersStem(yearStem, targetBranch) {
    const start = { 甲: 2, 己: 2, 乙: 4, 庚: 4, 丙: 6, 辛: 6, 丁: 8, 壬: 8, 戊: 0, 癸: 0 };
...
function buildLifePalace(pillars) {
    ...
    if (mIdx === undefined || hIdx === undefined)
        return null;
    ...
    return {
        ...
        hidden: exports.HIDDEN_STEMS[lpBranch] || [],
...
function buildBodyPalace(pillars, lifePalace) {
    if (!lifePalace)
        return null;
    ...
    return {
        ...
        element: el,
        element_th: ELEMENT_TH_MAP[el] || el,
        hidden: exports.HIDDEN_STEMS[bpBranch] || [],
...
function computeSiLingDays(year, month, day, hour, minute) {
    try {
        const tyme = require("tyme4ts");
        const st = tyme.SolarTime.fromYmdHms(year, month, day, hour + 1, minute, 0); // +1h: ICT→BJT
        let term = st.getTerm();
        for (let i = 0; i < 3 && !term.isJie(); i++)
            term = term.next(-1); // ถอยถึง 節 (เปลี่ยนเดือน)
...
function buildSiLing(monthBranch, daysSinceJie) {
    ...
    if (daysSinceJie != null) {
        ...
        for (let i = 0; i < table.length; i++) {
            if (daysSinceJie < acc + table[i].days) {
                idx = i;
                break;
            }
            acc += table[i].days;
...
function buildMinorLuck(pillars, gender, count = 6) {
    if (!pillars.hour)
        return null; // ต้องใช้ 時柱
    const ys = pillars.year.stem;
...
function buildConceptionPalace(pillars) {
    ...
    return {
        ...
        element: el,
        element_th: ELEMENT_TH_MAP[el] || el,
        hidden: exports.HIDDEN_STEMS[branch] || [],
...
function buildPalaceReadings(pillars) {
    const out = {};
    let yearHex = null;
    if (pillars) {
        try {
    ...
    for (const p of PALACE_TABLE) {
        const cell = { pillar: p.pillar, zh: p.zh, age: p.age, title_th: p.title_th, title_en: p.title_en, title_zh: p.title_zh };
...
function buildFiveStructure(pillars) {
