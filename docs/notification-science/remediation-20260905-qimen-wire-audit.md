...
## Independent review remediation: V4 action/purpose guards

The independent backend reviewer and parent subsequently found two pre-deployment gaps in the initial V4 contract: it accepted non-travel purposes, and it could verify a correctly labeled but weak selected hour pair as `recommended`. The initial shared August fixture selected N, whose door is 死 under ordinary policy or 囚 under Tongzong, despite a 旺 star. This was an invalid recommendation fixture; successful roundtrip checks alone did not prove action eligibility. No claim is made that this new-code gap affected production.

RED reproduced with the reviewer's unmodified `scripts/test-qimen-v4-contract-review.mts`: **70 checks, 18 failures**, confined to those two gaps. The fixes are V4-only:

- Full builder/verifier and compact parser require `purpose === "travel"`.
- The full builder/verifier requires both selected hour star and door vigor to be 旺 or 相. This is an existing producer requirement derivable directly from the full snapshot, not a newly relaxed or invented selection rule. Compact contains no vigor, so that criterion remains full-snapshot validation; provider building starts from a verified full snapshot.
- Legacy V2/V3 remain frozen. The independent reviewer also rebuilds the old fixtures against the actual pre-V4 module and compares complete canonical snapshot and compact bytes, beyond this implementation's stored digest goldens.

The base synthetic August fixture now selects an existing supportive pair: **W for ordinary; SE for Tongzong**. `selectSupportingDirection(value)` selects only a real existing 旺/相 pair, changes only the two input direction fields, and throws `qimen_fixture_no_supporting_direction` when no pair exists. It never rewrites state labels or arrangements.

Four original monthly matrix arrangements have no supportive pair (Tongzong 子/亥/巳/午); the test now explicitly proves both selection and full building fail for them. Only then does the separately named `arrangeSyntheticSupportingDoor(value)` test helper construct a different synthetic chart by exchanging one complete door tuple between noncentral palaces. Associated code, Chinese name, vigor and any intrinsic quality move together; no label is independently altered. This explicit test-only construction preserves the 24 positive monthly/profile cases and 216 full palace checks **in addition to four negative no-support cases**. It is not external-engine evidence, a prediction, or any production fallback.

GREEN: independent review **70/70**, updated V4 matrix, the five legacy snapshot/compact/detail/manifest suites, and seasonal occurrence-builder/scheduler/independent-root-review suites all pass. `git diff --check` passes. Updated supportive August fixture sizes are **1878 / 1888 B compact** and **17065 / 17083 B full snapshot** (ordinary/Tongzong). Earlier fixture byte measurements above describe the pre-remediation synthetic selections and are superseded by these values. Whole-provider limits remain unchanged and require their separate final transport tests.
