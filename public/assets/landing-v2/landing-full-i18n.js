(function () {
  ...
  const HTML_LANG = { th: 'th', en: 'en', zh: 'zh-Hant', vi: 'vi', ja: 'ja', ko: 'ko', ru: 'ru', es: 'es' };
  ...
  const ROWS = [
    ...
    [`Chart, Timing, Luopan และ AI Sifu ไม่แยกกันเป็นคนละเรื่อง`, `Chart, Timing, Luopan และ AI Sifu ไม่แยกกันเป็นคนละเรื่อง`, `Chart, Timing, Luopan, and AI Sifu stay connected.`, `命盤、擇時、羅盤與 AI Sifu 互相連動。`, `Chart, Timing, Luopan và AI Sifu được kết nối với nhau.`, `Chart、Timing、Luopan、AI Sifu が分断されません。`, `차트, 타이밍, 나경, AI Sifu가 연결됩니다.`, `Chart, Timing, Luopan и AI Sifu работают как единый контекст.`, `Chart, Timing, Luopan y AI Sifu quedan conectados.`],
    ...
    [`คุยต่อจากคำตอบเดิมโดยไม่ต้องเล่าดวงใหม่ เพราะบริบทถูกผูกไว้กับ profile และพื้นที่จริง`, `คุยต่อจากคำตอบเดิมโดยไม่ต้องเล่าดวงใหม่ เพราะบริบทถูกผูกไว้กับ profile และพื้นที่จริง`, `Continue from the previous answer without re-explaining your chart, because context is tied to your profile and real space.`, `可從原答案繼續追問，不必重講命盤，因背景已連到 profile 與真實空間。`, `Tiếp tục từ câu trả lời trước mà không cần kể lại lá số, vì bối cảnh đã gắn với hồ sơ và không gian thật.`, `プロフィールと実空間に背景が紐づくため、命盤を説明し直さずに続けられます。`, `맥락이 프로필과 실제 공간에 연결되어 있어 명식을 다시 설명하지 않고 이어갈 수 있습니다.`, `Продолжайте от предыдущего ответа без повторного описания карты: контекст связан с профилем и пространством.`, `Continúa desde la respuesta anterior sin explicar de nuevo la carta, porque el contexto queda unido al perfil y al espacio real.`],
    ...
    [`โปรไฟล์ดวง เวลา ทิศ และคำถามของคุณถูกใช้ร่วมกันในแต่ละหน้า ทำให้ Chart, Timing, Luopan และ AI Sifu ไม่แยกกันเป็นคนละเรื่อง`, `โปรไฟล์ดวง เวลา ทิศ และคำถามของคุณถูกใช้ร่วมกันในแต่ละหน้า ทำให้ Chart, Timing, Luopan และ AI Sifu ไม่แยกกันเป็นคนละเรื่อง`, `Your chart profile, timing, direction, and question are shared across pages, so Chart, Timing, Luopan, and AI Sifu do not drift apart.`, `你的命盤 profile、時間、方位與問題會在各頁共用，讓 Chart、Timing、Luopan 與 AI Sifu 不會各說各話。`, `Hồ sơ lá số, thời điểm, phương hướng và câu hỏi được dùng chung giữa các trang, nên Chart, Timing, Luopan và AI Sifu không tách rời.`, `命盤プロフィール、時期、方位、質問が各ページで共有されるため、Chart、Timing、Luopan、AI Sifu が分断されません。`, `명식 프로필, 시간, 방향, 질문이 페이지 간 공유되어 Chart, Timing, Luopan, AI Sifu가 따로 놀지 않습니다.`, `Профиль карты, время, направление и вопрос используются на всех страницах, поэтому Chart, Timing, Luopan и AI Sifu не расходятся.`, `Tu perfil, tiempo, dirección y pregunta se comparten entre páginas para que Chart, Timing, Luopan y AI Sifu no se separen.`],
    [`needs profile`, `ต้องมีโปรไฟล์`, `needs profile`, `需要 profile`, `cần hồ sơ`, `プロフィール必要`, `프로필 필요`, `нужен профиль`, `requiere perfil`],
    ...
    [`ใช้โปรไฟล์เดียวกันข้ามหน้า เพื่อลดคำตอบที่ขัดกันเองระหว่าง Chart, Timing, Luopan และ AI Sifu`, `ใช้โปรไฟล์เดียวกันข้ามหน้า เพื่อลดคำตอบที่ขัดกันเองระหว่าง Chart, Timing, Luopan และ AI Sifu`, `Uses one profile across pages to reduce conflicting answers between Chart, Timing, Luopan, and AI Sifu`, `跨頁使用同一 profile，減少 Chart、Timing、Luopan 與 AI Sifu 之間互相矛盾`, `Dùng một hồ sơ trên nhiều trang để giảm câu trả lời mâu thuẫn giữa Chart, Timing, Luopan và AI Sifu`, `同じプロフィールを各ページで使い、Chart、Timing、Luopan、AI Sifu の矛盾を減らします`, `하나의 프로필을 여러 페이지에서 사용해 Chart, Timing, Luopan, AI Sifu 간 모순을 줄입니다`, `Один профиль на всех страницах уменьшает противоречия между Chart, Timing, Luopan и AI Sifu`, `Usa un perfil en todas las páginas para reducir contradicciones entre Chart, Timing, Luopan y AI Sifu`],
  ...
  ROWS.forEach((row) => {
    LANGS.forEach((lang, index) => {
      dictionaries[lang].set(row[0], row[index + 1] || row[1] || row[0]);
    });
  });
  ...
  function normalizeLang(value) {
    ...
    if (raw === 'zh-hant' || raw === 'zh-tw' || raw === 'zh-hk' || raw === 'cn') return 'zh';
  ...
  function t(source, lang) {
    return dictionaries[lang].get(source) || dictionaries.th.get(source) || source;
  }
  ...
  function translateText(lang) {
    ...
    nodes.forEach((node) => {
      ...
      if (translated !== key || lang === 'th') node.textContent = replacePreservingTrim(source, translated);
    });
  }
  ...
  function translateAttrs(lang) {
    document.querySelectorAll('*').forEach((el) => {
      translatableAttrs.forEach((attr) => {
        ...
        if (translated !== source || lang === 'th') el.setAttribute(attr, translated);
      });
    });
  }
  ...
  function persistLang(lang) {
    try {
      ...
      if (LIVE_GLOBAL_LANGS.includes(lang)) {
        GLOBAL_STORAGE_KEYS.forEach((key) => window.localStorage.setItem(key, lang));
      }
    } catch (_) {}
  }
  ...
  function updateUrl(lang) {
    ...
    if (lang === 'th') url.searchParams.delete('lang');
  ...
  function boot() {
    ...
    window.HK_LANDING_LANG = { set: applyLang, get: () => normalizeLang(document.body.getAttribute('data-lang')) || 'th', supported: LANGS.slice() };
  }
