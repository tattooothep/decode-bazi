...
### 💡 KEY แก้ blocker (5-agent #2 · 27 พ.ค.) — 12長生 คือ discriminator
root model ง่าย (本氣/中氣/被沖) แยก 真化 vs 假從 ไม่ได้ เพราะ signature "中氣被沖" เหมือนกัน · **ตัวแยกจริง = 12長生 (墓/旺) ของรากที่ถูกสอบ:**
- F7 真化: 乙@辰 = **衰/墓** (餘氣อ่อน) → 辰戌冲 = สลายราก (沖開墓庫·ถอนทิ้ง) → rootless → 真化
- ไนท์ 假從: 己@午 = **臨官** (รากแข็ง) → 子午冲 = สะเทือนแต่ไม่ถอน → ยังมีราก → 假從
- ✅ มีใน `shared.twelvePhase()` + `STEM_POLARITY` (陰乙 ตามง่ายกว่า陽) แล้ว · proto ไม่เรียกใช้ → root model ใหม่ต้องผูก: ราก被沖 + 12長生(衰/墓/絕→ถอน · 臨官/帝旺/長生→ทน) + 陰陽干 · ไม่ใช่ปรับ threshold (signature เดียวกัน threshold ใดก็แยกไม่ได้)
