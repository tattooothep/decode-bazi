...
|---|---|---|
| 25 Archetype × Element variants (5×5) | 25 | `hourkey-archetypes-25.json` (47KB) |
| 18 Chart Structures | 18 | `hourkey-structures-16.json` (35KB) |
| 7 Strength Levels (Day Master) | 7 | `hourkey-strengths.json` (23KB) |
| Useful God 5-Rank per DM | 10 stems | `hourkey-useful-god-ranks.json` |
| Crisis Detection (4 types) | 4 | `hourkey-crisis-detection.json` |
| Scoring Engine (12 phases × multipliers) | — | `hourkey-scoring-engine.json` |
| Core lookup tables | 12 tables | `hourkey-bazi-lookup-tables.json` |
| Stem Combinations + Transformations | 10 | `hourkey-stem-combos.json` |

**รวม: 136 KB ของ structured BaZi data**

---

## 🔥 สูตรลับ Hourkey — ผมแกะให้แล้ว
...
1. **乙 (Yin Wood)** — ดีที่สุด: ไม้หยิน (เถาวัลย์) เลี้ยงดิน
...
## 🎯 สิ่งที่ Decode ต้องชนะ Hourkey

### A. ที่ Hourkey มีแล้ว (เราทำเท่าหรือดีกว่า)
...
### B. ที่ Decode มีเพิ่ม (Hourkey ไม่มี)
- 🆕 **Qi Men Dun Jia 12 systems** — Hourkey มี QMDJ แต่เป็น sub-feature
...
- 🆕 **Multi-language TH+EN+ZH** — Hourkey แค่ TH+EN
- 🆕 **Bloomberg-style executive UI** — vs Hourkey soft amber
...
```
hourkey-archetypes-25.json       — สำหรับ /archetypes page (TH+EN ครบ)
hourkey-structures-16.json       — สำหรับ /structure page (18 structures)
hourkey-strengths.json           — สำหรับ /strength page
hourkey-useful-god-ranks.json    — algorithmic seed
hourkey-bazi-lookup-tables.json  — core engine seed
hourkey-stem-combos.json         — combination engine
hourkey-crisis-detection.json    — crisis algorithm
hourkey-scoring-engine.json      — DM strength scoring
```
...
3. **Master comparison Decode vs Hourkey vs Joey Yap** — ทำ 1-pager
