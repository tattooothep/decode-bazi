...
export async function POST(req: NextRequest) {
  ...
  if (action === "extend_sub") {
    ...
    try {
      ...
      const row = await client.query<{ tier: string; sub_expires_at: string }>(
        `UPDATE users SET sub_expires_at = GREATEST(COALESCE(sub_expires_at, now()), now()) + ($2 || ' days')::interval
           WHERE id=$1 RETURNING tier,sub_expires_at::text`,
        [id, String(days)]
      );
      if (!row.rows[0]) throw new Error("not_found");
      after = row.rows[0].sub_expires_at;
      const tier = String(row.rows[0].tier || "").toLowerCase();
      if (tier === "premium" || tier === "master") {
        await client.query(
          `INSERT INTO product_entitlement_sources
           ...
           VALUES($1,'admin',$2,$3,'active',now(),$4::timestamptz,$5::jsonb)
           ON CONFLICT(source_kind,source_ref) DO UPDATE SET
             tier=EXCLUDED.tier,status='active',expires_at=EXCLUDED.expires_at,
             metadata=EXCLUDED.metadata,updated_at=now()`,
          [id, `admin:manual:${id}`, tier, after, JSON.stringify({ actor: admin.email })]
        );
        const selected = await recomputePaidEntitlement(client, id);
        after = selected?.sub_expires_at || after;
      }
      await client.query("COMMIT");
    } catch (error) {
      await client.query("ROLLBACK").catch(() => null);
      return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "extend_sub_failed" }, { status: 500 });
    } finally {
      client.release();
    }
    await writeAdminAudit({
      ...
      targetType: "user",
      targetId: id,
      payload: { before, after, days },
      ip,
      userAgent: ua,
    });
    return NextResponse.json({ ok: true, sub_expires_at: after });
  }
