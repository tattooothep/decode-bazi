...
export async function POST(req: Request) {
  ...
  const ipLimited = await rateLimit(
    `mobile-shrine-shop-offer-ip:${ip}`,
    90,
    RATE_WINDOW_MS,
  );
  ...
  const bearerLimited = await rateLimit(
    `mobile-shrine-shop-offer-bearer:${bearerFingerprint}`,
    45,
    RATE_WINDOW_MS,
  );
  ...
  const limited = await rateLimit(
    `mobile-shrine-shop-offer-user:${session.userId}`,
    30,
    RATE_WINDOW_MS,
  );
  ...
  let input;
  try {
    input = parseShrineOfferInput(await req.json());
  } catch {
    return json({ ok: false, error: "invalid_shrine_offer" }, 400);
  }
  try {
    const result = await offerShrineGrant(session.userId, input);
