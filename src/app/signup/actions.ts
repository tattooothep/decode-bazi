...
export async function signupAction(prev: { error?: string } | null, formData: FormData): Promise<{ error?: string }> {
  const email = String(formData.get("email") || "").trim().toLowerCase();
  const password = String(formData.get("password") || "");
  const name = String(formData.get("name") || "").trim();
  if (!email || !password) return { error: "กรอก email + password" };
  if (password.length < 6) return { error: "รหัสผ่านต้อง ≥ 6 ตัวอักษร" };

  const existing = await q1<{ id: string }>("SELECT id FROM users WHERE email=$1", [email]);
  if (existing) return { error: "อีเมลนี้สมัครไปแล้ว — กรุณาเข้าสู่ระบบ" };
