...
function readPalace(value, index) {
  ...
  if (!formationCodes || !warningCodes) return null;
  return Object.freeze({ ...record, formationCodes, warningCodes });
}
...
function readLayer(value, expectedKind, versions) {
  ...
  const palaces = palacesInput.map((palace, index) => readPalace(palace, index));
  if (palaces.some((palace) => !palace)) return null;
  return Object.freeze({ ...record, decisionRole: DECISION_ROLES[expectedKind], palaces: Object.freeze(palaces) });
}
...
function buildQimenThreeLayerSnapshot(input) {
  ...
  const sourceTuple = Object.freeze(Object.fromEntries(LAYER_KINDS.map((kind) => [kind, layers[kind].sourceCode])));
  const base = {
    snapshotSchema: 2,
    event: top.event,
...
function providerLayer(snapshot, kind) {
  ...
  return Object.freeze({
    ...
    sourceCode: snapshot.sourceTuple[kind],
