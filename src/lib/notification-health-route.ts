...
const { collectHealth: collectNotificationHealth } = require("./notification-observability.cjs") as {
...
export function authorized(req: Request, env: NodeJS.ProcessEnv): boolean {
...
export function readHeartbeat(file: string | undefined): string | null {
  if (!file) return null;
...
export async function notificationHealthPost(req: Request, dependencies: NotificationHealthRouteDependencies = {}) {
