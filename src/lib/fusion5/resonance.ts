...
/* ==================== R6 ปีชง 3 อารยธรรม ==================== */
export type R6Vote = { system: "bazi" | "western" | "vedic"; heavy: boolean | null; rationale: string; dates: string[] };
export type R6Person = {
  ...
  votes: R6Vote[];
  voiceCount: number;                         // 0-3 = จำนวนระบบที่บอกปีนี้หนัก
};
...
function buildR6(b: ResonanceBirth, targetYear: number, westernTl: WesternTimeline | null, vedicTl: VedicTimeline | null): R6Person {
...
function computePerson(b: ResonanceBirth, sciences: ResonanceScience[], targetYear: number): PersonResonance {
  ...
  return {
    ...
    r6: buildR6(b, targetYear, westernTlKeep, vedicTlKeep),
  };
}
...
export function renderResonanceBlockTh(res: FusionResonance): string {
  ...
  for (const p of res.perPerson) {
    ...
    if (p.r6) {
      const v = (s: "bazi" | "western" | "vedic") => p.r6!.votes.find((x) => x.system === s);
      const mark = (x?: R6Vote) => (x ? (x.heavy === true ? "✓หนัก" : x.heavy === false ? "✗" : "–ไม่มีข้อมูล") : "–");
      push(`  R6 ปีชง ${p.r6.targetGanzhi}: จีน=${mark(v("bazi"))} · ตะวันตก(เสาร์)=${mark(v("western"))} · พระเวท(SadeSati)=${mark(v("vedic"))} → ${p.r6.voiceCount}/3 เสียงบอกปีหนัก`);
      for (const x of p.r6.votes) push(`    · ${x.system === "bazi" ? "จีน" : x.system === "western" ? "ตะวันตก" : "พระเวท"}: ${x.rationale}`, 2);
    }
