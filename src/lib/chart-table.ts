...
/* #24 · 命宮 Life Palace · exact formula copied from load-profile.ts:728 */
...
export function buildLifePalace(pillars: Pillars): LifePalaceInfo | null {
  ...
  const lpIdx = (((14 - mIdx - hIdx) % 12) + 12) % 12;
  ...
  return {
    ...
    title_th: `กิ่งวังตน · ${BRANCH_TH_MAP[lpBranch] || lpBranch} · ธาตุ${ELEMENT_TH_MAP[el] || el}`,
    title_en: `Life Palace · ${lpBranch} · ${el}`,
    title_zh: `命宮 · ${lpBranch} · ${el}`,
  };
}
