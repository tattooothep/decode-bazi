...
function buildInterpretPrompt(input) {
  ...
  return [
    ...
    "4. pace=fast_this_period (จันทร์) คือสิ่งเดียวที่เฉพาะ 2 ชั่วโมงนี้ → ต้องเป็นหัวเรื่องของยามถ้ามี · pace=slow_background คือฉากหลังของหลายวัน ให้พูดว่า 'ช่วงนี้/หลายวันนี้' ห้ามเขียนว่าเฉพาะ 2 ชั่วโมงนี้หรือ 'เป็นพิเศษในยามนี้' · ถ้าไม่มีมุมจันทร์เลย ให้บอกใน body/meaning สั้นๆ ว่ายามนี้ไม่ต่างจากภาพรวมช่วงนี้ แล้วสรุปฉากหลัง — แต่ title ยังต้องเป็นสิ่งที่ควรทำ ห้ามตั้งหัวข้อว่า 'ยามนี้ไม่พิเศษ/ธรรมดา'",
    ...
    "9. title = สิ่งที่ควรทำ/ระวังในยามนี้ (ไม่ใช่รหัสดาว) มีชื่อดาวได้ไม่เกิน 1 ดวง · body ประโยคแรก = ทำอะไร/เลี่ยงอะไร ประโยคสอง = เพราะดาวอะไร · doList/avoidList ทุกข้อขึ้นต้นด้วยคำกริยา เป็นการกระทำจริงในชีวิตที่ทำได้ใน 2 ชั่วโมง ไม่ขัดกันเอง ห้ามข้อที่พูดถึงตัวคำทำนายเอง (เช่น 'อย่าคาดหวังว่ายามนี้จะพิเศษ') · ช่วงเวลาให้เรียกว่า '2 ชั่วโมงนี้' ไม่ใช่ 'ชั่วโมงนี้'",
