...
async function collectHealth(db, input = {}) {
  ...
  const [summaryResult, aggregatesResult] = await readOnlyBounded(db, (readDb) => Promise.all([
    readDb.query(
      `WITH scoped AS (
         SELECT a.*,l.kind
         ...
         SELECT count(*)::int AS overdue_count,
           ...
           FROM scoped WHERE status='retry_due' AND next_retry_at<=now()
       ), stale AS (
         SELECT count(*)::int AS stale_count FROM scoped
          WHERE (lease_token IS NOT NULL AND lease_expires_at<=now())
             OR (status='reserved' AND COALESCE(send_started_at,updated_at,created_at)<=now()-($3::text||' seconds')::interval)
       ), receipts AS (
         SELECT count(*)::int AS stalled_count,
                COALESCE(max(extract(epoch FROM now()-accepted_at)),0)::bigint AS oldest_age_seconds
           FROM scoped WHERE provider='expo' AND status='provider_accepted'
             AND provider_ticket_id IS NOT NULL AND provider_receipt_checked_at IS NULL
             AND accepted_at<=now()-($2::text||' seconds')::interval
       ), readiness AS (
         SELECT count(*) FILTER (WHERE t.id IS NULL OR (a.provider='fcm' AND (t.device_push_token IS NULL OR t.platform='ios' OR COALESCE(t.device_token_type,'')='apns')) OR (a.provider='expo' AND t.expo_push_token IS NULL))::int AS token_mismatch_count,
                count(*) FILTER (WHERE a.provider='fcm')::int AS active_fcm_count,
                count(*) FILTER (WHERE a.provider='expo')::int AS active_expo_count
           FROM scoped a
          LEFT JOIN mobile_push_tokens t ON t.id=a.token_id AND t.enabled=true
         ...
         SELECT count(*) FILTER (WHERE status='dead')::int AS dead_letter_count,
           ...
           FROM scoped
       ) SELECT retry.overdue_count,retry.oldest_age_seconds,stale.stale_count,receipts.stalled_count,
                receipts.oldest_age_seconds AS receipt_oldest_age_seconds,readiness.token_mismatch_count,
                readiness.active_fcm_count,readiness.active_expo_count,
                terminal.dead_letter_count,terminal.invalid_token_count,terminal.uncertain_count
           FROM retry CROSS JOIN stale CROSS JOIN receipts CROSS JOIN readiness CROSS JOIN terminal`,
      [String(config.lookbackHours), String(config.thresholds.receiptStallSeconds), String(config.thresholds.staleAttemptSeconds)],
    ),
    readDb.query(
  ...
  const activeProviders = { fcm: numeric(summary.active_fcm_count), expo: numeric(summary.active_expo_count) };
  ...
  const metrics = {
    retry: { overdueCount: numeric(summary.overdue_count), oldestAgeSeconds: numeric(summary.oldest_age_seconds) },
    leases: { staleCount: numeric(summary.stale_count) },
    receipts: { stalledCount: numeric(summary.stalled_count), oldestAgeSeconds: numeric(summary.receipt_oldest_age_seconds) },
    readiness: {
      tokenMismatchCount: numeric(summary.token_mismatch_count), credentialMismatchCount,
      mismatchCount: numeric(summary.token_mismatch_count) + credentialMismatchCount,
    },
    outcomes: { deadLetterCount: numeric(summary.dead_letter_count), invalidTokenCount: numeric(summary.invalid_token_count), uncertainCount: numeric(summary.uncertain_count) },
    worker,
    scheduler,
...
async function reconcile(db, input = {}) {
  const { lookbackHours } = optionsFor(input);
  const result = await readOnlyBounded(db, (readDb) => readDb.query(
    `WITH parents AS (
       SELECT l.id,l.delivery_status,
        ...
        WHERE l.updated_at >= now()-($1::text||' hours')::interval
...
module.exports = { collectHealth, reconcile, readOnlyBounded };
